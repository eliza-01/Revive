...
class BuffAfterRespawnWorker:
    def init(
            ...
            debug: bool = False,
            check_is_dead: Optional[Callable[[], bool]] = None, # ново
            wait_alive_timeout_s: float = 1.0, # ново
    ):
        ...
        def run_once(self) -> bool:
            ...
            # гард: не бафаемся в смерти
            if not self._wait_until_alive(self._wait_alive_timeout_s):
                self._on_status("[buff] still dead, abort", False)
                return False

            with DASHBOARD_GUARD.session():
                if self._flow:
                    return self._run_flow(win)
                # fallback-сценарий без flow
        ...
        def _load_cfg(self):
            ...
            # алиас: допускаем имя dashboard_blocked вместо dashboard_is_locked
        ...
        def _run_flow(self, win: Dict) -> bool:
            ...
            for idx, step in enumerate(self._flow, start=1):
                ...
                if op == "click_any":
                    ...
                    # Если блок виден, пробуем левым кликом раз в interval_s до исчезновения блока.
                    zone_key = step["zone"]
                    tpl_key = step["tpl"]  # ожидаем "dashboard_is_locked" или алиас
                    timeout_ms = int(step.get("timeout_ms", 8000))
                    interval_s = float(step.get("probe_interval_s", 1.0))

                    # алиас к "dashboard_blocked"
